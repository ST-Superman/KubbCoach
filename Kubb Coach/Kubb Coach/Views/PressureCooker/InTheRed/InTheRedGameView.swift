...
struct InTheRedGameView: View {
            ...
            ToolbarItem(placement: .topBarTrailing) {
                Button { undoLastRound() } label: {
                    Image(systemName: "arrow.uturn.backward")
                        .foregroundStyle(roundScores.isEmpty ? Color(.tertiaryLabel) : KubbColors.phasePressureCooker)
                }
    ...
    private func roundDot(for round: Int) -> some View {
        ...
        return Circle()
            ...
            .overlay(
                Circle()
                    .strokeBorder(isCurrent ? KubbColors.phasePressureCooker : Color.clear, lineWidth: 1.5)
    ...
    private func dotColor(score: Int?, isCurrent: Bool) -> Color {
        guard let s = score else {
            return isCurrent ? KubbColors.phasePressureCooker.opacity(0.3) : Color(.tertiarySystemFill)
        }
        switch s {
        case 1:  return KubbColors.swedishGold
        case 0:  return KubbColors.forestGreen.opacity(0.6)
    ...
    private func scenarioCard(for scenario: InTheRedScenario) -> some View {
        ...
        .overlay(
            RoundedRectangle(cornerRadius: 14)
                .strokeBorder(KubbColors.phasePressureCooker.opacity(0.2), lineWidth: 1)
        )
    }
    ...
    private var buttonColor: Color {
        ...
        default: return KubbColors.phasePressureCooker
        }
    }
